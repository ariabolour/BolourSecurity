...
Define the small set of types, protocols, and guarantees that every other BlurSecurity module speaks — so that a protection policy means the same thing in `BlurKeychain` and `BlurSecureStorage`, an error behaves the same way in every `catch`, and a secret is never printable anywhere.
...
- Nothing else. Core has no features — it has vocabulary. Anything with behavior belongs in a capability module (there is deliberately no `BlurUtilities`; see [ADR-0005](../adr/0005-consolidated-module-set.md)).
...
public protocol TrustEvaluating: Sendable {
    // Conformed to by BlurCertificates.TrustEvaluator; consumed by BlurNetworkSecurity.
}

public protocol AttestationProviding: Sendable {
    // Conformed to by BlurAppIntegrity.AttestationService; consumed by apps' networking layers.
}
...
Foundation, `os`. Nothing else — Core is the bottom of the graph and imports no BlurSecurity module.
