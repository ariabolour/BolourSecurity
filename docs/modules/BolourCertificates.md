...
Give developers a typed view of certificates and chains, a trust evaluator with honest semantics, and a pinning policy whose *shape* enforces operational best practice (SPKI pins, mandatory backup pins) — so that the difference between "TLS" and "TLS to the party I meant" becomes a value the app declares and tests. `BolourNetworkSecurity` consumes this module's policies; this module owns their meaning.
...
/// Proof of evaluation — the only chain type BolourNetworkSecurity accepts as trusted.
...
The prose above says this in places; this section exists so it's findable in one place, because
the DER/X.509 parser is a high-risk piece of code and its scope is deliberately narrow **by
design, not by omission**. `BolourCertificates` does **not** implement, and will not grow into:

- **Certificate chain validation** — path building, chain-of-trust verification, and
  cross-certification are entirely `SecTrust`'s job. This module never walks or validates a
  chain itself.
- **Hostname verification** — matching a presented certificate's SANs against the host you
  intended to reach is `SecPolicyCreateSSL` + `SecTrust`'s job, not this module's.
- **Revocation checking** — CRL/OCSP evaluation is `SecTrust`'s job (and, on Apple platforms,
  partly the OS's own CT/revocation infrastructure). This module surfaces no revocation status.
- **Trust-anchor policy** — which roots are trusted is the system trust store's decision (or an
  explicit test-anchor override in test code only); this module never ships or manages a root
  set.
- **A replacement for TLS trust** — `PinningPolicy`/`TrustEvaluator` are additive *on top of*
  system trust, never a substitute for it; a pinned-but-untrusted chain always fails (see
  `TrustEvaluator.evaluate`'s own ordering — system trust is checked first, unconditionally).

What this module *does* own: DER structural parsing, SPKI extraction, certificate metadata
inspection (subject/issuer/validity/SANs), and pin derivation/comparison. If a feature request
would expand this module into an independent trust engine, that is a signal to say no, not a
roadmap item.

## Dependencies

`BolourSecurityCore`, `BolourCrypto` (SPKI hashing); Apple: Security (`SecTrust`, `SecCertificate`), Foundation.
...
- Identity (`SecIdentity`) import/export and client-certificate support (v2.0, with BolourNetworkSecurity mTLS).
