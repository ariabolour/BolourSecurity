...
- Producing `AuthenticatedContext` — the ecosystem's presence token — consumed by `BolourKeychain` reads and `SecureEnclaveKey` operations so users authenticate once per logical operation, not once per API call.
...
`BolourSecurityCore`; Apple: LocalAuthentication, Foundation.
...
- **Honest limits:** biometrics gate *access on this device*; they are not proof of identity to a server (that's `BolourAppIntegrity` + `BolourOAuth` territory, and the docs draw this line in the security-considerations article). Apps must also handle the OS-level truth that a device passcode holder can enroll new biometry.
