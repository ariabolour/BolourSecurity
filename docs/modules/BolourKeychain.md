...
Out of scope: certificates/identities (BolourCertificates at v2.x), internet passwords with autofill semantics (roadmap), encrypted blobs above item size norms (BolourSecureStorage's job).
...
`BolourSecurityCore`; Apple: Security (Keychain Services), LocalAuthentication (context pass-through only), Foundation.
...
- `PresenceRequirement != .none` builds a `SecAccessControl`; reads of gated items accept an `AuthenticatedContext` (BolourBiometrics' LAContext wrapper, passed through a Core seam) to reuse a fresh authentication instead of double-prompting.
...
- **Honest limits:** keychain items survive app uninstall on iOS (documented prominently — apps must decide first-run policy; the docs article shows the recommended `removeAllSecrets()`-on-fresh-install pattern). Keychain data is per-device-class protected by the OS; we do not add redundant app-layer encryption here — that's `BolourSecureStorage`'s layer.
