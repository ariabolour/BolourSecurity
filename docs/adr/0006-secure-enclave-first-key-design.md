...
2. Modules that need a device-bound key create SE keys by default: `BolourAppIntegrity` uses App Attest's own SE-resident keys today. `BolourSecureStorage` vault master keys are *intended* to be SE-wrapped under this same decision — as of this writing they are not yet; they're software-held in the keychain (see [BolourSecureStorage's module doc](../modules/BolourSecureStorage.md) and [CHANGELOG's Known limitations](../../CHANGELOG.md#known-limitations)). Recorded here precisely so the gap between decision and delivery stays visible rather than getting silently assumed away by a document that only states intent.
