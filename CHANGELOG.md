...
- **`BolourSecurityIntegrationApp` v1** ([README](BolourSecurityIntegrationApp/README.md)): a SwiftUI app, `xcodegen`-generated with no team baked in, giving a manual-verification screen for Keychain, Secure Enclave, Biometrics, App Attest, and OAuth against real APIs on a real device or simulator. Verified running in iOS Simulator, including a genuinely interesting finding: `SecureEnclaveKey.create(tag:)` *succeeds* there, unlike the bare `swift test` host process (`errSecMissingEntitlement`) — a real, code-signed app bundle apparently gets a more permissive Simulator path than an unentitled test binary does. Documented clearly in the app's README as proof the code path works, **not** proof of the hardware security property, since Simulator has no physical Secure Enclave. This is the first real external consumer of the umbrella product's `@_exported import` (ADR-0008). It's a manual-verification aid, not an automated CI test — it doesn't host the `.requiresDevice` XCTest suites, which remains outstanding.
