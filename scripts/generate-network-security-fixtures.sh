...
# Requires OpenSSL 3.x. Re-run to regenerate. Nothing here needs a literal pin updated
# afterwards — NetworkFixtures.swift derives every pin from the DER it just loaded — but the
# passphrase below is duplicated in NetworkFixtures.pkcs12Passphrase and must move with it.
set -euo pipefail
...
openssl pkcs12 -export \
  ...
  -out "$OUT/server-identity.p12" -passout pass:boloursecurity-test-fixture \
...
openssl req -x509 -new -key "$W/decoy.key" -out "$W/decoy.pem" \
  -not_before 20200101000000Z -not_after 20500101000000Z \
  -subj "/CN=decoy.boloursecurity.test" \
  -addext "subjectAltName=DNS:decoy.boloursecurity.test"
