...
| `BolourBiometrics` | Beta | Thin mapping over `LAContext`, small policy surface — but **zero automated real-biometric-hardware test coverage**, not gated, genuinely absent (see [Integration Testing](docs/IntegrationTesting.md)). Three watchOS compile failures in this module went unnoticed until an actual CI run caught them, which argues against "freeze-ready" on its own |
| `BolourSecureStorage` | Beta | The vault's master key is software-held in the keychain, not Secure Enclave-wrapped as [ADR-0006](docs/adr/0006-secure-enclave-first-key-design.md) intends — a real gap between design and delivery. The docs now say so consistently; stays Beta until the gap itself is closed |
