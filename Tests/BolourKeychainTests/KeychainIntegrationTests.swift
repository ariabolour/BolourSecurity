...
///
/// Tagged `.requiresDevice` per `docs/IntegrationTesting.md`'s test tiering — this suite is
/// Tier 3 (device-required): it currently *runs* against the simulator/macOS host when the
/// probe succeeds there, but the tag exists so a real device-hosted test plan (once
/// `BolourSecurityIntegrationApp` exists) can select it explicitly rather than relying on the
/// probe's runtime skip alone.
@Suite("Keychain integration", .enabled(if: KeychainProbe.isAvailable), .tags(.requiresDevice))
struct KeychainIntegrationTests {
    ...
    private func makeKeychain(_ label: String = #function) -> Keychain {
        Keychain(service: "BolourSecurityTests.\(label).\(UUID().uuidString)")
    }
