...
/// constraint as `BlurKeychainTests`' real-keychain suite.
@Suite("SecureEnclaveKey", .enabled(if: SecureEnclaveProbe.isAvailable))
struct SecureEnclaveKeyTests {

    private func uniqueTag() -> String { "BlurCryptoTests.se-key.\(UUID().uuidString)" }
