...
struct SigningTests {
    ...
    func p256() throws {
        let key = SigningKey<BlurCrypto.P256>.software()
    ...
    func codable() throws {
        let verificationKey = SigningKey<BlurCrypto.P256>.software().verificationKey
        let encoded = try JSONEncoder().encode(verificationKey)
        let decoded = try JSONDecoder().decode(VerificationKey<BlurCrypto.P256>.self, from: encoded)
