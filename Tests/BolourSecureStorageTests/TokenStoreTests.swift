...
/// constraint `BolourKeychainTests` documents (live SecItem round-trips can't run in hostless
/// `swift test` bundles).
///
/// Tagged `.requiresDevice` per `docs/IntegrationTesting.md` — Tier 3 (device-required).
@Suite("TokenStore", .enabled(if: KeychainProbe.isAvailable), .tags(.requiresDevice))
struct TokenStoreTests {

    private func makeStore() -> TokenStore {
        TokenStore(
            keychain: Keychain(service: "BolourSecureStorageTests.tokens"),
