...
| Integration sample app | **Partial** | `BolourSecurityIntegrationApp` v1 exists ([README](../BolourSecurityIntegrationApp/README.md)) — simulator-scope, manual verification only, doesn't host the `.requiresDevice` XCTest suites yet (see below). `BolourSecurityDemo` isn't built at all |
...
**Built:**
- **`BolourSecurityIntegrationApp` v1** — a SwiftUI app (`xcodegen`-generated, no team baked in) with a manual-verification screen per surface: Keychain, Secure Enclave, Biometrics, App Attest, OAuth. Verified running in iOS Simulator: real Keychain round-trips, real `SecureEnclaveKey` create/sign/verify, live `BiometryAvailability` reporting, an honest `.unsupported(.simulator)` for App Attest. Closes "you can see it run" — not "CI runs it automatically," below.

**Specified, not built — ready for a dedicated follow-up:**
- **`BolourSecurityDemo`** — one polished example app demonstrating a realistic integration: Keychain token storage, biometric-gated retrieval, Secure Enclave signing, encrypted file storage, certificate pinning, OAuth PKCE sign-in, App Attest — teaching architecture, not API syntax. Distinct from `BolourSecurityIntegrationApp`, which validates APIs one at a time.
- **Hosting the actual XCTest bundles inside `BolourSecurityIntegrationApp`** — v1 is a manual-verification UI, not a test runner. Closing the "unentitled `xctest` host" gap means the three `.requiresDevice` suites need to run inside this app's process, not just have their APIs poked by hand.
