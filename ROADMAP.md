...
| Integration sample app | **Partial** | `BolourSecurityIntegrationApp` v1 exists ([README](../BolourSecurityIntegrationApp/README.md), simulator-scope, manual-verification UI for Keychain/Secure Enclave/Biometrics/App Attest/OAuth) — but it does not yet host the actual XCTest bundles for the three `.requiresDevice` suites (still a separate, unbuilt piece — see below). `BolourSecurityDemo` is specified but not built at all. |
...
**Built:**
- **`BolourSecurityIntegrationApp` v1** — a SwiftUI app (`xcodegen`-generated, no team baked in — every developer signs with their own Apple ID) with a manual-verification screen per surface: Keychain, Secure Enclave, Biometrics, App Attest, OAuth. Verified running in iOS Simulator: real Keychain round-trips, real `SecureEnclaveKey` create/sign/verify, live `BiometryAvailability` reporting, and an honest `AttestationSupport.unsupported(.simulator)` result for App Attest. This closes the "you can at least *see* it run" gap — it does not close the "CI runs it automatically" gap below.

**Specified, not built — ready for a dedicated follow-up:**
- **`BolourSecurityDemo`** — one polished example app (not ten) demonstrating a realistic integration: Keychain token storage, biometric-gated retrieval, Secure Enclave signing, encrypted file storage (Vault), certificate pinning, OAuth PKCE sign-in, and the App Attest lifecycle, with clear production/demo boundaries. Teaches architecture, not just API syntax. Distinct from `BolourSecurityIntegrationApp` above, which validates APIs one at a time rather than teaching a realistic integration.
- **Hosting the actual XCTest bundles inside `BolourSecurityIntegrationApp`** — its v1 is a manual-verification UI, not a test runner. Closing the "unentitled `xctest` host" gap that currently skip-gates every hardware-backed round-trip in CI means the three `.requiresDevice` suites need to actually execute inside this app's process (or an app-hosted test target alongside it), not just have their APIs poked by hand.
