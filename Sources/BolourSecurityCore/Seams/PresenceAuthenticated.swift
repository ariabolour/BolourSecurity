...
///
/// `BolourBiometrics` (Layer 2) will provide the concrete conformer — an `LAContext` wrapper —
/// so a caller can perform one authentication and reuse it, rather than triggering a second
/// biometric prompt. `BolourKeychain` and `BolourCrypto` (Layer 1) accept `any PresenceAuthenticated`
