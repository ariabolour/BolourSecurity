...
public struct SecureEnclaveKey: Sendable {
    ...
    public static func load(tag: String) throws(CryptoError) -> SecureEnclaveKey? {
        ...
        // Apple documents a `kSecReturnRef` query against `kSecClassKey` as returning a `SecKey`,
        // but `CFTypeRef` carries no Swift-checkable static type — `as?` against a toll-free-
        // bridged CF type compiles as an unconditional success (confirmed by the compiler's own
        // "will always succeed" diagnostic on that form), so it is not a real runtime check.
        // `CFGetTypeID` is the actual documented way to verify a CFTypeRef's dynamic type; this is
        // defense-in-depth against Apple's own documented contract, not an expected failure mode.
        guard CFGetTypeID(result) == SecKeyGetTypeID() else {
            throw CryptoError.secureEnclaveOperationFailed(underlying: errSecParam)
        }
        // swiftlint:disable:next force_cast
        return SecureEnclaveKey(secKey: (result as! SecKey), tag: tag)
    }
