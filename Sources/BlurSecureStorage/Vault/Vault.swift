...
///   yet Secure-Enclave-wrapped (`BlurCrypto.SecureEnclaveKey` doesn't exist yet either — this
...
public actor Vault {
    ...
    private static let fileKeyInfo = Data("blur.securestorage.file-key.v1".utf8)
    ...
    /// `context` from `BlurBiometrics` throws ``StorageError/masterKeyUnavailable(reason:)``.
    public static func open(
        ...
        let keychain = Keychain(service: "BlurSecureStorage.vault", protection: protection)
        let masterKeyItemKey = ItemKey("blur.securestorage.vault.\(name.rawValue).masterkey")
    ...
    /// platform (see `BlurKeychainTests`'s `KeychainProbe` gate); this is how the file
    ...
    static func openForTesting(
        ...
        return Vault(
            ...
            keychain: Keychain(service: "BlurSecureStorageTests"), masterKeyItemKey: "test.masterkey",
    ...
    private static let manifestKeySalt = Data("blur.securestorage.manifest-key.v1".utf8)
    ...
    private static func directoryURL(for name: VaultName) throws(StorageError) -> URL {
        ...
        return appSupport.appendingPathComponent("BlurSecurity/Vaults/\(name.rawValue)", isDirectory: true)
    }
